...
export default {
  ...
  setup () {
    ...
    // 2. Field 需要进行数据绑定 字段名称最好和后台接口的一致
    // 3. 定义name属性指定的校验规则函数，Form的 validation-schema 接受定义好的校验规则对象
    ...
    // 1.发送验证码
    // 1.1 绑定发送验证码按钮点击事件
    // 1.2 校验手机号，如果成功就发送短信（API），请求成功开始倒计时，不能再次点击
    // 1.3 如果失败，失败得校验样式显示出来
    const send = async () => {
      ...
      if (valid === true) {
        ...
        if (time.value === 0) {
          ...
          Message({ type: "success", text: "短信发送成功！" });
          // 开启倒计时
          time.value = 60;
          resume();
        }
      } else {
        // 失败，使用vee的错误函数显示错误信息 setFieldError(字段，错误消息)
